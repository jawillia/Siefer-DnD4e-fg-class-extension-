...
function addClassSpecificFeatures(sClassName, rAdd, sClassFeatureName, sClassFeatureFilteredDescription, sClassFeatureOriginalDescription, nLevel, sFullDescriptionText)
	switch(sClassName:upper(), 
	{
		...
		--NOTE:Tested up to level 29
		["PALADIN (CAVALIER)"] = function() return addPaladinCavalierFeatures(sClassName, rAdd, sClassFeatureName, sClassFeatureFilteredDescription, sClassFeatureOriginalDescription, nLevel, sFullDescriptionText) end,
		--NOTE:Tested up to level 10
...
function addClassSpecificPreFeatures(sClassName, rAdd, sDescriptionText, tClassFeatures, nLevel, sFullDescriptionText)
	switch(sClassName:upper(), 
	{
		...
		["PALADIN (CAVALIER)"] = function() return addPaladinCavalierPreFeatures(sClassName, rAdd, sDescriptionText, tClassFeatures, nLevel, sFullDescriptionText) end,
		...
		["SORCERER (ELEMENTALIST)"] = function() return addSorcererElementalistPreFeatures(sClassName, rAdd, sDescriptionText, tClassFeatures, nLevel, sFullDescriptionText) end
	});
end
...
function getChosenPrefeature(rAdd, sClassFeatureOriginalDescription, sPrefeatureType, sClassFeatureName)
	if not sClassFeatureOriginalDescription or not sPrefeatureType or not rAdd then
		return;
	end
	local tCurrentFeatures = DB.getChildren(rAdd.nodeChar, "specialabilitylist");
	...
	for _, featureNode in pairs(tCurrentFeatures) do
		...
		--Name equals "X Elementalist"
		if not foundString then
			sNamePattern = "(%a+) Elementalist";
			foundString = string.match(DB.getText(DB.getPath(featureNode, "value")):lower(), sNamePattern:lower());
		end
		if not foundString then
...
function displayAlternativeFeatureDialog(rAdd, tAlternativeClassFeatures, tStringCharBuildData, fnCallback)
	...
	local tDialogData = {
		...
		custom = { rAdd=rAdd, tAlternativeClassFeatures=tAlternativeClassFeatures, fnCallback=fnCallback },
	};
...
function callbackResolveClericChannelDivinity(tSelection, rAdd, tSelectionLinks)
	...
	if #tSelection == 1 then
		...
		local sChannelDivinityDescription = "You gain the " .. sChannelDivinityAbilityName .. " channel divinity power. You can only use one of them per encounter.";
		addBasicClassFeature(rAdd, sChannelDivinityTitleName, sChannelDivinityDescription);
...
function addFighterWeaponmasterFeatures(sClassName, rAdd, sClassFeatureName, sClassFeatureFilteredDescription, sClassFeatureOriginalDescription)
	...
	elseif sClassFeatureName == "Combat Challenge" then
		addDefaultClassFeature(sClassName, rAdd, sClassFeatureName, sClassFeatureFilteredDescription, sClassFeatureOriginalDescription);
		CharClassPowerManager.addFighterCombatChallengePower(rAdd);
...
function callbackResolveFightingStyleDialogSelection(tSelection, tData)
	...
	if #tSelection == 1 then
		...
		addBasicClassFeature(tData.rAdd, tSelection[1], nil, "powerdesc", sSelectedFightingStyleDBReference);
	...
	if sClassFeatureName == "Scoundrel Weapon Talent" then
		...
		for _, featureNode in pairs(tCurrentFeatures) do
			if DB.getText(DB.getPath(featureNode, "value")) == "Sharpshooter Talent" then
				...
				displayAlternativeFeatureDialog(rAdd, tAlternativeClassFeaturesOptions, tStringCharBuildData, fnCallbackWrapper);
				break;
			end
		end
...
---------------------------------------------
function addPaladinCavalierPreFeatures(sClassName, rAdd, sDescriptionText, tClassFeatures, nLevel, sFullDescriptionText)
...
function callbackResolvePaladinCavalierPreFeatureSelection(tSelection, tData)
	...
	addBasicClassFeature(tData.rAdd, tSelection[1], sVirtueDescription);

	CharClassManager.addEssentialsClassFeatures(tData.rAdd, nil, tData.sFullDescriptionText, tData.sClassName, tData.nLevel, tData.sFullDescriptionText, true);
end
function addPaladinCavalierFeatures(sClassName, rAdd, sClassFeatureName, sClassFeatureFilteredDescription, sClassFeatureOriginalDescription, nLevel, sFullDescriptionText)
	local tCurrentFeatures = DB.getChildren(rAdd.nodeChar, "specialabilitylist");
	...
	--Replace older power at certain levels
	if string.find(sClassFeatureName:lower(), "level %d+ cavalier daily power")
...
function callbackResolveDruidProtectorPreFeatureSelection(tSelection, tData)
	...
	addBasicClassFeature(rAdd, sCircleName, sCircleDescription);
...
--------------------------------------------------
function addSorcererElementalistPreFeatures(sClassName, rAdd, sDescriptionText, tClassFeatures, nLevel, sFullDescriptionText)
	local sClassFeatureName = "Elemental Specialty";
	local sClassFeatureDescriptionPattern = "<p>%s*<b>%s*" .. sClassFeatureName.. "%s*</b></p>%s*(.-)<p>Ability Scores</p>";
	local sClassFeatureSpecificDescriptionText = string.match(sDescriptionText, sClassFeatureDescriptionPattern);

	addBasicClassFeature(rAdd, sClassFeatureName, removeLinkLists(sClassFeatureSpecificDescriptionText));

	if sClassFeatureSpecificDescriptionText then
		local fnCallbackWrapper = function()
			CharClassManager.addEssentialsClassFeatures(rAdd, nil, sFullDescriptionText, sClassName, nLevel, sFullDescriptionText, true);
		end			
		displayClassFeatureSelectionsDialog(rAdd, sClassFeatureSpecificDescriptionText, sClassFeatureName, nil, nil, fnCallbackWrapper);
	end
end
function addSorcererElementalistFeatures(sClassName, rAdd, sClassFeatureName, sClassFeatureFilteredDescription, sClassFeatureOriginalDescription)
	...
	if sClassFeatureName == "Elemental Specialty" then
		addBasicClassFeature(rAdd, sClassFeatureName, sClassFeatureFilteredDescription);
		displayClassFeatureSelectionsDialog(rAdd, sClassFeatureOriginalDescription, sClassFeatureName);
	elseif sClassFeatureName == "Escalating Elements" then
		addBasicClassFeature(rAdd, sClassFeatureName, sClassFeatureFilteredDescription);
		CharClassPowerManager.addPreFeaturePower(rAdd, sClassFeatureOriginalDescription, "elemental specialty", sClassFeatureName, sSubFeatureDescriptionText)
	else
		addDefaultClassFeature(sClassName, rAdd, sClassFeatureName, sClassFeatureFilteredDescription, sClassFeatureOriginalDescription);
	end
end
